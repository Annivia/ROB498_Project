...
if __name__ == "__main__":
    ...
    if (reward_type == 'euclidean'):
        ...
        env = PandaDiskPushingEnv_euclidean(visualizer=True, render_non_push_motions=True,  camera_heigh=800, camera_width=800, render_every_n_steps=10)
    elif (reward_type == 'euclidean_bar'):
        register_env("PandaDiskPushingEnv", create_environment_euclidean_bar)
        env = PandaDiskPushingEnv_euclidean_bar(visualizer=True, render_non_push_motions=True,  camera_heigh=800, camera_width=800, render_every_n_steps=10)
    elif (reward_type == 'square'):
        register_env("PandaDiskPushingEnv", create_environment_square)
        env = PandaDiskPushingEnv_square(visualizer=True, render_non_push_motions=True,  camera_heigh=800, camera_width=800, render_every_n_steps=10)
    elif (reward_type == 'square_bar'):
        register_env("PandaDiskPushingEnv", create_environment_square_bar)
        env = PandaDiskPushingEnv_square_bar(visualizer=True, render_non_push_motions=True,  camera_heigh=800, camera_width=800, render_every_n_steps=10)
    else:
