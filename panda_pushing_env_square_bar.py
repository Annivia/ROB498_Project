...
TARGET_POSE_FREE = np.array([0.75, 0.1])
TARGET_POSE_OBSTACLES = np.array([0.75, -0.1])
OBSTACLE_RADIUS = 0.05
OBSTACLE_CENTRE = np.array([0.575, -0.05])


class PandaDiskPushingEnv_square_bar(gym.Env):
    """
    ...
    def __init__(self, debug=False, visualizer=None, include_obstacle=True, render_non_push_motions=True,
        ...
        self.frames = []
        print("Called: Reward: Square Bar")
    ...
    def reset(self, random_start=False):
        ...
        if self.include_obstacle:
            self.obstacleUid = p.loadURDF(self.object_file_path,
                                          basePosition=[OBSTACLE_CENTRE[0], OBSTACLE_CENTRE[1], 0],
    ...
    def _is_done(self, state):
        ...
        if self.done_at_goal:
            if self.include_obstacle:
                ...
                at_goal = np.linalg.norm(state[:2] - TARGET_POSE_FREE[:2]) < 1.2 * DISK_RADIUS
    ...
    def render_frame(self):
        if self.debug:
            ...
            if self.is_render_on:
                ...
                cv2.imshow('Rendered Image', rgb_img)
                cv2.waitKey(1)  # Update the window every 1ms
        else:
            pass
...
if __name__ == '__main__':
    ...
    env = PandaDiskPushingEnv_square_bar(debug=script_args.debug, include_obstacle=script_args.obstacle)
    env.reset()
